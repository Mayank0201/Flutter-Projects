...
class _GameHudOverlayState extends State<GameHudOverlay> {
  ...
  @override
  void dispose() {
    // This overlay is the sole owner of onStateChanged while it's mounted; clear it
    // on the way out so a disposed State is never retained/invoked by the long-lived
    // FlowGridGame, and so a later overlay doesn't silently inherit a stale callback.
        ...
        children: [
          ...
          ValueListenableBuilder<int>(
            ...
            builder: (context, week, _) =>
                _miniStat('WEEK $week', Colors.white),
          ),
          ...
          ValueListenableBuilder<int>(
            ...
            builder: (context, score, _) =>
                _animatedStat('$score', Colors.amber),
          ),
          _miniLabel('SCORE'),
  ...
  /// Continuous calm→amber→dusty-rose gradient instead of a hard red/green
  /// snap at the 0.4 threshold, so crossing it reads as a gradual shift
  /// rather than a jump-cut.
  Color _satisfactionColor(double sat) {
    const low = Color(0xFFC97575); // dusty rose
    const mid = Color(0xFFD9A05B); // muted amber
    const high = Color(0xFF8FB89A); // calm sage
    final t = sat.clamp(0.0, 1.0);
    if (t < 0.5) {
  ...
  /// The HUD's "MENU" button previously exited to the main menu instantly, with
  /// no confirmation and no chance to save — one misclick discarded the run.
  /// This pauses, confirms, and gives the player a way back out if they hit it
  /// by accident.
  Future<void> _confirmExitToMenu() async {
    ...
    final confirmed = await showDialog<bool>(
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: Text(
          'Unsaved progress in this run will be lost. Use SAVE first if you want to keep it.',
    ...
    switch (map) {
      case MapType.zen:
        items = [
          _helpItem(
            'BALANCED CLIMATE',
            'Standard traction and physics. A peaceful sandbox environment without harsh environmental hazards.',
          ),
        ];
        break;
      case MapType.andes:
        items = [
          _helpItem(
            'TERRACOTTA CANYONS',
            'Sparse connection corridors. Valley routes are critical.',
          ),
          _helpItem(
            'MOUNTAIN ARCHITECTURE',
            'Buildings are decorated with clay slate chimneys.',
          ),
        ];
        break;
      case MapType.nile:
        items = [
          _helpItem(
            'RIVER BASIN',
            'Bridges are highly critical to cross the wide central river splitting the map.',
          ),
          _helpItem(
            'OASIS MUD-BRICK',
            'Buildings feature flat clay domes representing desert oasis architecture.',
          ),
        ];
        break;
      case MapType.arctic:
        items = [
          ...
          _helpItem(
            'SNOWY ROOFS',
            'Buildings feature thick white snow caps on their roofs.',
          ),
        ];
        break;
      case MapType.savanna:
        items = [
          ...
          _helpItem(
            'DUST STORMS',
            'Desert dust storms periodically reduce visibility and slow all traffic.',
          ),
          _helpItem(
            'THATCHED ROOFS',
            'Buildings feature gold straw thatch overlays.',
          ),
        ];
        break;
      case MapType.delta:
        items = [
          ...
          _helpItem(
            'WETLANDS IVY',
            'Ivy/moss grows on lower building corners and roof edges.',
          ),
        ];
        break;
    }
