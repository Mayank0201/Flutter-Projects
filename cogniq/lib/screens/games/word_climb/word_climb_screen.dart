...
const List<CrossClimbLevel> _kLevels = [
  ...
  CrossClimbLevel(
    clues: ['Opposite of cold', 'A warm weather wind maker', 'Metal canister', 'A taxi vehicle', 'Feline pet'],
    answers: ['HOT', 'HAT', 'FAT', 'CAT', 'CAB'],
  ),
  CrossClimbLevel(
    clues: ['Move fast on foot', 'Amusement or enjoyment', 'Fish breathing limb', 'Healthy and in good shape', 'Rest on a chair'],
    answers: ['RUN', 'FUN', 'FIN', 'FIT', 'SIT'],
  ...
  CrossClimbLevel(
    clues: ['Paper guide of the world', 'Tool to clean the floor', 'Fizzy soda drink', 'Cooking vessel', 'Adverb of negation'],
    answers: ['MAP', 'MOP', 'POP', 'POT', 'NOT'],
  ),
  CrossClimbLevel(
    clues: ['Opposite of wet', 'Attempt or endeavor', 'Plaything for children', 'Male child', 'A body of water along coast'],
    answers: ['DRY', 'TRY', 'TOY', 'BOY', 'BAY'],
  ),
  CrossClimbLevel(
    clues: ['Opposite of cold', 'Fenced entryway', 'Gave a gift', 'Underground stone chamber', 'A box or folder instance'],
    answers: ['LATE', 'GATE', 'GAVE', 'CAVE', 'CASE'],
  ),
  CrossClimbLevel(
    clues: ['A narrow road lane', 'Opposite of early', 'Calendar date', 'Fenced entryway', 'Gave a gift'],
    answers: ['LANE', 'LATE', 'DATE', 'GATE', 'GAVE'],
  ),
  CrossClimbLevel(
    clues: ['Greeting card', 'Hospital room ward', 'Moderately hot temperature', 'Creepy crawler in soil', 'Do physical or mental labor'],
    answers: ['CARD', 'WARD', 'WARM', 'WORM', 'WORK'],
  ),
  CrossClimbLevel(
    clues: ['Daring or courageous', 'Unable to walk easily', 'Opposite of early', 'Narrow street or road lane', 'Large body of water'],
    answers: ['TAME', 'LAME', 'LATE', 'LANE', 'LAKE'],
  ),
  CrossClimbLevel(
    clues: ['Large sea boat', 'Accidentally fall or slide', 'Glided along a surface', 'Slender or thin body', 'Shut a door violently'],
  ...
  CrossClimbLevel(
    clues: ['Sugary tasting candy', 'Fabric piece on a bed', 'Woolly farm animal', 'Rest state of slumber', 'Rising or falling sharply'],
    answers: ['SWEET', 'SHEET', 'SHEEP', 'SLEEP', 'STEEP'],
  ),
  CrossClimbLevel(
    clues: ['Opposite of dark', 'Engage in physical combat', 'Ability to see with eyes', 'Opposite of day', 'Great strength or power'],
    answers: ['LIGHT', 'FIGHT', 'SIGHT', 'NIGHT', 'MIGHT'],
  ...
  CrossClimbLevel(
    clues: ['Baking powder ingredient', 'Surface we walk on in a room', 'Overflow of water', 'Red fluid in veins', 'Family of young birds'],
    answers: ['FLOUR', 'FLOOR', 'FLOOD', 'BLOOD', 'BROOD'],
  ),
  CrossClimbLevel(
    clues: ['Home building dwelling', 'Small rodent animal', 'Awaken from sleep', 'Path or roadway travel', 'Throw or expel'],
    answers: ['HOUSE', 'MOUSE', 'ROUSE', 'ROUTE', 'ROUTS'],
  ),
  CrossClimbLevel(
    clues: ['Liquid we drink', 'Flicker or hesitate', 'One who rescues or stores', 'Cut off or divide', 'High body temperature'],
  ...
  CrossClimbLevel(
    clues: ['Man\'s best friend pet', 'Felled tree timber piece', 'A large amount or batch', 'Deep cooking vessel', 'Opposite of cold'],
    answers: ['DOG', 'LOG', 'LOT', 'POT', 'HOT'],
  ),
];

class WordClimbScreen extends StatefulWidget {
  const WordClimbScreen({super.key});
  @override
  State<WordClimbScreen> createState() => _WordClimbScreenState();
}

class _WordClimbScreenState extends State<WordClimbScreen> {
  int _levelIndex = 0;
  late CrossClimbLevel _level;
  late List<bool?> _results; // null = unchecked/neutral, true = correct, false = wrong
  bool _won = false;
  int _hintCount = 0;
  late final List<CrossClimbLevel> _levels;
  ...
  void initState() {
    super.initState();
    _levels = _kLevels.where((l) => l.answers.first.length >= 4).toList();
    _level = _levels[0];
  ...
  Future<void> _useHint() async {
    ...
    final currentText = _controllers[idx - 1].text.toUpperCase().trim();
    final targetWord = _level.answers[idx].toUpperCase();
    ...
    setState(() {
      _hintCount = newCount;
      final hintText = _getHintText(currentText, targetWord);
      _controllers[idx - 1].text = hintText;
      
      if (hintText == targetWord) {
        _results[idx] = true;
      }
  ...
  void _loadLevel([SharedPreferences? prefs]) {
    _level = _levels[_levelIndex % _levels.length];
    final numSlots = _level.answers.length - 2;
  ...
  void _onKey(String key) {
    ...
    // Find the currently focused controller/focusNode
    int focusedIdx = -1;
    ...
    if (focusedIdx == -1) {
      // If none is focused, auto-focus the first empty slot or just the first slot
    ...
    final controller = _controllers[focusedIdx];
    final answerWord = _level.answers[focusedIdx + 1];
    ...
    if (key == '⌫') {
      if (controller.text.isNotEmpty) {
        setState(() {
          ...
          _results[focusedIdx + 1] = null;
          _saveState();
        });
      } else if (focusedIdx > 0) {
        // Go back to previous slot and delete its last character
        _focusNodes[focusedIdx - 1].requestFocus();
        final prevController = _controllers[focusedIdx - 1];
        if (prevController.text.isNotEmpty) {
          setState(() {
            prevController.text = prevController.text.substring(0, prevController.text.length - 1);
            _results[focusedIdx] = null;
            _saveState();
          });
        }
      }
    } else if (key == '↵') {
      // Enter: if the current slot is filled, move to next slot or check answers
      if (controller.text.length == wordLen) {
        if (focusedIdx < _controllers.length - 1) {
          _focusNodes[focusedIdx + 1].requestFocus();
        } else {
          _check();
        }
      ...
      if (controller.text.length < wordLen) {
        setState(() {
          ...
          // Auto-advance cursor to next slot if this slot is filled
          if (controller.text.length == wordLen && focusedIdx < _controllers.length - 1) {
            _focusNodes[focusedIdx + 1].requestFocus();
          }
        });
      }
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        actions: [
          ...
          Padding(padding: const EdgeInsets.only(right: 12), child: Center(child: Text('Level ${_levelIndex + 1}', style: AppTheme.numberStyle(color: AppTheme.crossclimbPurple, fontSize: context.scale(13))))),
        ],
      ),
      body: SafeArea(
        child: Column(
          children: [
                ...
                child: Column(
                  children: [
                    ...
                    if (_won) ...[
                      ...
                      AutoNextCountdown(
                        onNext: _nextLevel,
                        accentColor: AppTheme.crossclimbPurple,
                      ),
                    ],
                  ],
                ),
            ...
            const SizedBox(height: 12),
            if (!_won) _CrossClimbKeyboard(onKey: _onKey),
          ],
        ),
      ),
    );
...
class _CrossClimbKeyboard extends StatelessWidget {
  final void Function(String) onKey;
  const _CrossClimbKeyboard({required this.onKey});
  static const _rows = [
    ['Q', 'W', 'E', 'R', 'T', 'Y', 'U', 'I', 'O', 'P'],
    ['A', 'S', 'D', 'F', 'G', 'H', 'J', 'K', 'L'],
    ['↵', 'Z', 'X', 'C', 'V', 'B', 'N', 'M', '⌫'],
