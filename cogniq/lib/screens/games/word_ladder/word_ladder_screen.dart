...
class _WordLadderScreenState extends State<WordLadderScreen> {
  ...
  bool _won = false;
  int _hintCount = 0;
  late final List<List<String>> _levels;

  @override
  void initState() {
    super.initState();
    _levels = _kLevels.take(50).toList();
  ...
  void _loadLevel() {
    _chain = _levels[_levelIndex % _levels.length];
        ...
        children: [
          ...
          AnimatedContainer(
            ...
            decoration: BoxDecoration(
              ...
              borderRadius: BorderRadius.circular(12),
              border: Border.all(
                color: isConnected ? Colors.green.withOpacity(0.6) : context.textMuted.withOpacity(0.2),
                width: isConnected ? 2 : 1,
              ),
              boxShadow: isConnected
                  ? [BoxShadow(color: Colors.green.withOpacity(0.15), blurRadius: 6, spreadRadius: 1)]
                  : AppTheme.cardShadow,
            ),
            child: ListTile(
              ...
              trailing: AnimatedSwitcher(
                ...
                child: isConnected
                    ? const Icon(Icons.check_circle, key: ValueKey('valid'), color: Colors.green)
                    : const Icon(Icons.error_outline, key: ValueKey('invalid'), color: Colors.orange),
              ),
            ),
          ),
        ],
        ...
        actions: [
          ...
          Padding(
            ...
            child: Center(
              child: Text(
                'Level ${_levelIndex + 1}',
                style: AppTheme.numberStyle(color: AppTheme.weaverBlue, fontSize: context.scale(13)),
              ),
            ),
          ),
        ],
              ...
              if (_won) ...[
                ...
                Center(
                  child: AutoNextCountdown(
                    onNext: _nextLevel,
                    accentColor: AppTheme.weaverBlue,
                  ),
                ),
              ],
