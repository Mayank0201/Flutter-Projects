...
class ColourLinkScreen extends StatefulWidget {
  const ColourLinkScreen({super.key});
  @override
  State<ColourLinkScreen> createState() => _ColourLinkScreenState();
}

class _ColourLinkScreenState extends State<ColourLinkScreen> {
  ...
  bool _isModActive(String name) {
    ...
    return _currentLevel >= RotationEngine.modifierStartLevel('colourlink') &&
        _activeModifiers.contains(name);
  }

  // COGNIQ-FIX:mod-getters
  bool get _isMonochromeActive => _isModActive('monochrome');
  bool get _isGridSizePairCountActive => _isModActive('gridSize_pairCount');
  bool get _isWallsActive => _isModActive('walls');
  bool get _isTortuosityActive => _isModActive('tortuosity');
  ...
  bool get _modsOn => !_playDailyMode && RotationEngine.hasModifiers('colourlink', _currentLevel);
  ...
  Future<void> _initLevelState() async {
    ...
    final savedLvl = prefs.getInt(PrefsKeys.gameLevel('colour_link')) ?? 0;
  ...
  String _getModifierDescription(String mod) {
    switch (mod) {
      case 'gridSize_pairCount':
        return 'Grid size and color pair count are expanded.';
      case 'walls':
        return 'Obstacle wall cells block path connections.';
      case 'tortuosity':
        return 'Paths wind with more complex turns.';
      case 'timer':
        return 'Link all colors before time runs out.';
      case 'monochrome':
        return 'All paths display identical monochrome styling.';
      default:
        return '';
    }
  }
    ...
    setState(() {
      ...
      if (!_playDailyMode && _currentLevel >= 30) { // not-a-modifier-gate
        if (_currentLevel < 45) {
          ...
          // From 45 the board is at its hardest (8x8 with 8 colours) and stays there.
          _gridSize = 8;
          _numColors = 8;
        }
      ...
      while (attempts < 2000) {
        ...
        if (!stuck) {
          ...
          if (pathsValid && _isTortuosityActive) {
  ...
  /// True when every step of [path] moves to an orthogonally adjacent cell.
  ///
  /// Checking only the endpoints is not enough: using a hint can delete cells
  /// out of the middle of another colour's path, leaving a path whose ends are
  /// still correct but which is broken in the middle. Without this it counted
  /// as a win.
  bool _isPathContiguous(List<int> path) {
    for (int i = 1; i < path.length; i++) {
      final a = path[i - 1];
      final b = path[i];
      final dr = (a ~/ _gridSize) - (b ~/ _gridSize);
      final dc = (a % _gridSize) - (b % _gridSize);
  ...
  void _checkSilentWin() {
    ...
    for (int c = 1; c <= _numColors; c++) {
      ...
      if (!_isPathContiguous(path)) allConnected = false;

      // Two colours sharing a cell would also inflate the filled-cell count.
      for (final cell in path) {
        if (!occupied.add(cell)) allConnected = false;
      }
    }

    if (allConnected) {
      final int requiredCells = _isTortuosityActive
  ...
  Future<void> _onLevelCleared() async {
    ...
    final prefs = await SharedPreferences.getInstance();
    final key = PrefsKeys.gameLevel('colour_link');
  ...
  void _nextLevel() async {
    ...
    await ProgressGuard.saveLevel('colour_link', _currentLevel, isDaily: false);
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        title: const GameTitle('Colour Link'),
        ...
        actions: [
          ...
          GameLevelChip(
            ...
            accent: AppTheme.accentFor('colour_link'),
