...
class _ColorFloodScreenState extends State<ColorFloodScreen> {
  ...
  bool _isModActive(String name) {
    ...
    return _currentLevel >= RotationEngine.modifierStartLevel('colorflood') &&
        _activeModifiers.contains(name);
  }

  // COGNIQ-FIX:mod-getters
  bool get _isFogActive => _isModActive('fog');
  bool get _isCenterSeedActive => _isModActive('centerSeed');
  bool get _isChaosActive => _isModActive('chaos');
  ...
  String _getModifierDescription(String mod) {
    switch (mod) {
      case 'centerSeed':
        return 'The flood originates from the center of the grid.';
      case 'timer':
        return 'Flood the grid before time expires.';
      case 'chaos':
        return 'Colors shuffle every 4 moves.';
      case 'fog':
  ...
  bool get _modsOn => !_playDailyMode && RotationEngine.hasModifiers('colorflood', _currentLevel);
  ...
  Future<void> _initLevelState() async {
    ...
    final savedLvl = prefs.getInt(PrefsKeys.gameLevel('color_flood')) ?? 0;
  ...
  Future<void> _onLevelCleared() async {
    ...
    final prefs = await SharedPreferences.getInstance();
    final key = PrefsKeys.gameLevel('color_flood');
  ...
  void _nextLevel() async {
    ...
    await ProgressGuard.saveLevel('color_flood', _currentLevel, isDaily: false);
  }
  ...
  void _flood(int targetColor) {
    ...
    setState(() {
      _movesLeft--;
      _movesPlayed++;
      if (_isChaosActive && _movesPlayed > 0 && _movesPlayed % 4 == 0) {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        title: const GameTitle('Color Flood'),
        ...
        actions: [
          ...
          GameLevelChip(
            ...
            accent: AppTheme.accentFor('color_flood'),
