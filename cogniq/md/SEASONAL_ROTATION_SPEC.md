...
>
> **Rejected twice, independently, on 2026-08-22** — once when the S3 framework was built for
> 2.1 and again when the winter event content was built for 2.2. Nothing from it shipped.
> It is kept only as a record of a design that was considered and turned down.
>
> **Why it was rejected:**
>
> 1. **It is a rewrite of the daily challenge system.** It plans changes to
>    `daily_challenge_manager.dart`, `game_info.dart`, `daily_screen.dart` and
>    `home_screen.dart`. `md/remember.md` §A puts the daily system off limits, and S3 was
>    approved to be built *alongside* it, not to replace it.
> 2. **Its time handling is the exact bug class it should avoid.** It computes
>    `today.difference(installDate).inDays` — consecutive local midnights are 23 or 25 hours
>    apart across a DST boundary — and treats `install_date` as UTC where it must be local.
>    `StreakManager` (S1, shipped 2.0) already fixed this properly; follow that instead.
> 3. **"Add ~17–20 new games as disabled definitions" is dead.** Half its list (KenKen,
>    Binairo, Futoshiki) was **deleted by owner decision on 2026-08-22**, and it counts 18
>    existing games when there are 33 entries / 23 live.
> 4. **Its 15-day season blocks and recap screen are a rotation feature, not an event
>    feature** — they belong to S2 (2.3) if anywhere.
> 5. **Its file paths are stale** (`d:/Mayank/Flutter-Projects/cogniq`).
>
> **What replaced it:** `lib/utils/seasonal_event_manager.dart` — four annually recurring
> events on a quarterly rhythm, anchored to the 18th of Dec/Mar/Jun/Sep. Specified in
> `md/RELEASE_PLAN.md` §4.3 and ruled in `md/remember.md` §12b.
>
> **The one idea worth keeping** was "a deterministic per-user unit of time". It was kept and
> re-anchored to the local calendar date, because a seasonal event that ignores the real
> calendar is not seasonal.
